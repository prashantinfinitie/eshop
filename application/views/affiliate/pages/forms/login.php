<?php if (ALLOW_MODIFICATION == 0) { ?>
    <div class="alert alert-warning">
        Note: If you cannot login here, please close the codecanyon frame by clicking on x Remove Frame button from top right corner on the page or <a href="<?= base_url('/affiliate') ?>" target="_blank" class="text-danger">>> Click here << </a>
    </div>
<?php } ?>
    ...
    <div class="modal-dialog  edit-modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('affiliate/auth/verify-account') ?>" id="verify-acount-form" class='form-submit-event' method="post">
    ...
    <div class="container-fluid ">
        <div class="card-body login-card-body">
            <div class="login-logo">
                <a href="<?= base_url() . 'affiliate/login' ?>"><img src="<?= base_url() . $logo ?>"></a>
            </div>
            <div class="row">
                <h2 class="text-dark">Welcome Back!</h2>
                <p class="text-dark mb-4">Please login to your account</p>
            </div>
            <form action="<?= base_url('auth/login') ?>" class='form-submit-event add_affiliate_user_form_1' method="post">
                <div class="mb-3">
                    <input type='hidden' name='<?= $this->security->get_csrf_token_name() ?>' value='<?= $this->security->get_csrf_hash() ?>'>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        <input type="password" class="form-control form-input passwordToggle" name="password" id="password" placeholder="Enter Your Password"
            ...
            <div>
                <a href="<?= base_url('affiliate/auth/sign_up') ?>" class="text text-danger font-weight-bold">Don't have any account?</a>
            </div>
        </div>
        <!-- /.login-card-body -->
    </div>
