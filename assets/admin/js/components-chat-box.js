...
let loadFromId = ''; // Use 'let' instead of 'const'
let ChatOffset = ''; // Use 'let' instead of 'const'
let ChatMsg = ''; // Use 'let' instead of 'const'
var isChatFetching = false;

function loadChat(from_id, type, offset = '', limit = '', sort = '', order = '') {
    
    isChatFetching = true;
    loadFromId = from_id; // Now this will work because 'loadFromId' is declared with 'let'
    offset = (!!offset) ? offset : 0;
    limit = (!!limit) ? limit : 10;
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (chats['error'] != true) {
                if (!!loaded_chat[type + '_' + from_id] && loaded_chat[type + '_' + from_id] != undefined) {
                    loaded_chat[type + '_' + from_id] = loaded_chat[type + '_' + from_id]=(chats['msg']);
                ...
                offset = chats['total_msg'] - 3; 

                // Store the updated offset for the next call
                ChatOffset = offset;
                ChatMsg = chats['msg'];
