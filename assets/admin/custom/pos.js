...
function display_products(products) {
    ...
    if (products !== null && products.length > 0) {
        for (i = 0; i < products.length; i++) {
            var allOutOfStock = false;
            var hasLowStock = false;
            var showStockStatus = true;

            var stock_type = products[i].stock_type;

            // If stock_type is null, do not show stock status
            if (stock_type === null || stock_type === undefined) {
                showStockStatus = false;
            } else if (stock_type == 0) {
                // Product level stock
                var stock = parseInt(products[i].stock);
                ...
                for (j = 0; j < variants.length; j++) {
                    var vStock = parseInt(variants[j]['stock']);
                    if (vStock > 0) {
                        allOutOfStock = false;
                        if (vStock <= 5) {
                            hasLowStock = true;
            ...
            if (allOutOfStock) {
                display_products += '<button class="btn btn-xs btn-danger shop-item-button p-1 text-white" type="button" disabled>Out of Stock</button>';
            } else if (hasLowStock) {
                display_products += '<button class="btn btn-xs btn-warning shop-item-button p-1 text-white" type="button" disabled>Low Stock</button>';
            } else {
                display_products += '<button class="btn btn-xs btn-info shop-item-button p-1" onclick="add_to_cart(event)" type="button">Add to Cart</button>';
            }
            display_products +=
                ' </div>' +
                ' </div>' +
...
$('#pos_form').on('submit', function (e) {
    ...
    if (confirm('Are you sure? Want to check out?')) {
        ...
        var submit_btn = $('#place_order_btn');
        var btn_html = $('#place_order_btn').html();
        var btn_val = $('#place_order_btn').val();
